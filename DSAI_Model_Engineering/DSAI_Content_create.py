...
#     img_to_bytes("DeepSphere_Logo_Final.png")
# )
# st.markdown(
...
def draft_content(filename):
    document = Document(filename)
    content = ''
    for paragraph in document.paragraphs:
        if "URL" in paragraph.text:
            content += "\n\n"
        if len(paragraph.text.split(' '))>15:
            text = paragraph.text
            if text.startswith("-"):
                text = text.replace("-","")
            text = text.replace("|","")
...
def main():
    Topic = st.text_input('Press ENTER after your input:')
    i=1
    #if len(Topic)>0:
    eu_col1,eu_col2,eu_col3 = st.beta_columns((1,1,2))
    eu_col1.success('Run Model')
    eu_col2.success('Step 1')
    if eu_col3.button("Identify&Rank Top 10 URLs for the given topic"):
...
#            clean_links = Extract_Ranked_urls(links)
    ...
    dc_col1,dc_col2,dc_col3 = st.beta_columns((1,1,2))
    dc_col1.success('Run Model')
    dc_col2.success('Step 2')
    if dc_col3.button("Extract appropriate content from the URLs for the  given topic"):
...
##################### To add logo ###############################################
        ...
        ##### add logo in Zoned header
            ...
            logo_path = 'DeepSphere_Logo_Final.png'    # Path of the image file
...
#            clean_links = Extract_Ranked_urls(links)
    ...
    pe_col1,pe_col2,pe_col3 = st.beta_columns((1,1,2))
    pe_col1.success('Run Model')
    pe_col2.success('Step 3')
    if pe_col3.button("Preprocessing the extracted content"):
        st.write('Implementation in-progress')
        pass
    pdf_col1,pdf_col2,pdf_col3 = st.beta_columns((1,1,2))
    pdf_col1.success('Run Model')
    pdf_col2.success('Step 4')
    if pdf_col3.button("Download raw content as pdf"):
        st.write('Implementation in-progress')
        pass
    dco_col1,dco_col2,dco_col3 = st.beta_columns((1,1,2))
    dco_col1.success('Run Model')
    dco_col2.success('Step 5')
    try:
        if dco_col3.button("Download raw Content as txt file"):
                quest = "Content Generation"
                filename = "Model Output - " + str(Topic)+".docx"        
                text_content = draft_content(filename)        
                st.markdown(download_link(text_content, 'draft_content.txt', 'Click here to download your output!',quest),unsafe_allow_html=True)
